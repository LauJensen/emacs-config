...
(setq my-roam-folder   (expand-file-name "~/Documents/Roam"))
...
;  (ido-mode 'both)  ; User ido mode for both buffers and files
  ...
  (dolist (p '("/usr/local/bin" "/home/lau/bin"))
...
;; Commented to give preference to elpaca
...
(use-package spinner :ensure t)
  (use-package yascroll
    ...
    (use-package unicode-fonts
      ...
      :config
      (unicode-fonts-setup)
      ;; After unicode-fonts has done its thing, force Nerd Font for PUA ranges.
      (when (member "Symbols Nerd Font Mono" (font-family-list))
        (let ((nerd (font-spec :family "Symbols Nerd Font Mono")))
          ;; Main Private Use Area
          (set-fontset-font t '(#xE000 . #xF8FF) nerd nil 'prepend)
          ;; Supplementary PUAs (some Nerd glyphs live here too)
          (set-fontset-font t '(#xF0000 . #xFFFFD) nerd nil 'prepend)
          (set-fontset-font t '(#x100000 . #x10FFFD) nerd nil 'prepend))))
...
;;      (set-face-foreground 'doom-modeline-buffer-file "#a83800")
    ...
    ;;  (use-package forge               :ensure t :after magit) currently an incompatability, magit it 4.20, forge requires 4.2.1
  ...
  ;;           ("C-M-$" . jinx-languages)))
    ...
    ;; These are loaded by elpaca now
...
(add-hook 'clojure-mode-hook
   (lambda ()
     (add-hook 'before-save-hook #'align-namespace nil t)))
...
(setq lsp-headerline-breadcrumb-enable-diagnostics nil) ;; No need for ugliness
...
(global-set-key (kbd "C-æ G") 'consult-ripgrep)
