...
Older installation/initialization:
...
TODO Change to your browser of choice - Using eww
...
Avoid gaps when resizing (untangled, goes in early-init.el)
...
(let* ((variable-tuple
        (when (display-graphic-p)
          (cond ((x-list-fonts "Iosevka") '(:font "Iosevka-15"))
                (t (warn "Cannot find Iosevka. Install it or change font.")
                   nil))))
  ...
  (with-eval-after-load 'org
    ;; Make sure the whole heading line is colorized (not just the text)
    (setq org-fontify-whole-heading-line t)

    (dolist (spec '((org-level-1 . "#ff6c6b")  ; 1 star
                    (org-level-2 . "#98be65")  ; 2 stars
                    (org-level-3 . "#51afef")  ; 3 stars
                    (org-level-4 . "#c678dd")  ; 4 stars
                    (org-level-5 . "#da8548")  ; etc…
                    (org-level-6 . "#46d9ff")
                    (org-level-7 . "#4db5bd")
                    (org-level-8 . "#bbbbbb")))
...
save it by running ~save-chat~ (disabled)
...
Install rust-mode and prepare for eglot support:
...
better by mixing with deeper-blue (disabled)
